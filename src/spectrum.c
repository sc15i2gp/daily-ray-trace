...
void free_spd(new_spectrum spd)
{
...
void new_const_spectrum(new_spectrum dst, f64 value)
{
    for(u32 i = 0; i < number_of_spectrum_samples; ++i) dst.samples[i] = value;
}
...
void new_spectral_mul_by_spectrum(new_spectrum dst, new_spectrum src0, new_spectrum src1)
{
    for(u32 i = 0; i < number_of_spectrum_samples; ++i)
    {
        dst.samples[i] = src0.samples[i] * src1.samples[i];
...
void new_spectral_mul_by_scalar(new_spectrum dst, new_spectrum src, f64 d)
{
    for(u32 i = 0; i < number_of_spectrum_samples; ++i)
    {
        dst.samples[i] = src.samples[i] * d;
...
void new_spectral_acc(new_spectrum dst, new_spectrum src)
{
    for(u32 i = 0; i < number_of_spectrum_samples; ++i)
    {
        dst.samples[i] += src.samples[i];
...
void new_spectral_acc_mul_by_scalar(new_spectrum dst, new_spectrum src, f64 d)
{
    for(u32 i = 0; i < number_of_spectrum_samples; ++i)
    {
        dst.samples[i] += src.samples[i] * d;
...
void new_spectral_sum(new_spectrum dst, new_spectrum src0, new_spectrum src1)
{
    for(u32 i = 0; i < number_of_spectrum_samples; ++i)
    {
        dst.samples[i] = src0.samples[i] + src1.samples[i];
...
rgb_f64 new_spectrum_to_xyz(new_spectrum spd, new_spectrum cmf_x, new_spectrum cmf_y, new_spectrum cmf_z, new_spectrum ref_white)
{
    rgb_f64 xyz = {0.0, 0.0, 0.0};
    f64 n = 0.0;
    for(u32 i = 0; i < number_of_spectrum_samples; ++i)
...
{
    rgb_f64 spd_xyz = new_spectrum_to_xyz(spd, cmf_x, cmf_y, cmf_z, ref_white);
...
void new_rgb_f64_to_spectrum(   rgb_f64 rgb,         new_spectrum dst, 
                            new_spectrum white_spd, new_spectrum red_spd, 
                            new_spectrum green_spd, new_spectrum blue_spd,
                            new_spectrum cyan_spd,  new_spectrum magenta_spd,
                            new_spectrum yellow_spd)
{
    new_spectrum rgb_spectra[] = {red_spd,  green_spd,   blue_spd};
    new_spectrum cmy_spectra[] = {cyan_spd, magenta_spd, yellow_spd};
    f64 rgb_values[] =        {rgb.r,    rgb.g,       rgb.b};
    u32 indices[] = {0, 1, 2};
    u32 tmp_val;
    ...
    f64 rgb_diff_mid_small = rgb_values[mid_index] - rgb_values[small_index];
    f64 rgb_diff_large_mid = rgb_values[large_index] - rgb_values[mid_index];
    new_spectral_mul_by_scalar(dst, white_spd, rgb_values[small_index]); //dst = white_rgb_spd * smallest_rgb
    new_spectral_acc_mul_by_scalar(dst, cmy_spectra[small_index], rgb_diff_mid_small); //dst += cym_spd * (mid - small)
    new_spectral_acc_mul_by_scalar(dst, rgb_spectra[large_index], rgb_diff_large_mid); //dst += rgb_spd * (large - mid)
}

f128 new_compute_blackbody_power(f128 temperature, f128 wavelength)
{
    const f128 c = 2.99792458e8L; //Speed of light
    const f128 h = 6.626176e-34L; //Planck's constant
    const f128 k = 1.380662e-23L; //Boltzmann constant
...
void new_generate_blackbody_spectrum(new_spectrum spd, f64 temperature)
{
    f128 temp_long = (f128)temperature;
    for(u32 i = 0; i < number_of_spectrum_samples; ++i)
    {
        f128 wavelength_in_nm = (f128)(smallest_wavelength + (i * sample_interval));
        f128 wavelength_in_m = wavelength_in_nm * 1e-9L;
        f128 blackbody_power = new_compute_blackbody_power(temp_long, wavelength_in_m);
        spd.samples[i] = (f64)(blackbody_power * 1e9L);
    }
}
...
//  - Each line is a single sample
//  - A sample consists of two csvs: "wavelength, value"
u32 new_load_csv_file_to_spectrum(new_spectrum dst, const char *csv_path)
{
    HANDLE csv_file_handle = CreateFile(csv_path, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    if(csv_file_handle == INVALID_HANDLE_VALUE) return 0;
