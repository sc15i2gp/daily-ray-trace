...
//DOING:
// - Right now
// 	- Simplify (even with incorrect output)
// 		- Modify algorithms to work with current spec of NEW_Scene_Path_Point
// 		- Change geometry sampling to separate sampling and pdf functions (even with repeated sqrt)
// 		- Move pdf calculations to compute_path_radiance
// 		- Remove NEW_
// 		- Remove currently unused code
// 		- Remove Ray struct
// 		- Remove spectrum buffer
// 	- Refactored output doesn't match previous output, figure out why
// 		- Figure out whether previous output was wrong/less correct
// 		- Output RGB representations of spectral data
// 		- Find a way to compare path calculations
...
int WINAPI WinMain(HINSTANCE instance, HINSTANCE prev_instance, LPSTR cmd_line, int show_cmd_line)
{
	...
	render_image(target_buffer, RENDER_TARGET_WIDTH, RENDER_TARGET_HEIGHT, 4);
